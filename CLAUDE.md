...
```
xiaonai/
├── bridge.py                 # ★ 消息桥接主进程：NapCat → 批处理 → LLM → 回复
├── scheduler_v5.py           # ★ 定时推送守护进程（systemd 托管，9 类推送）
├── bot.py                    # NoneBot 入口（加载 src/plugins/*）
├── config.py                 # pydantic BotConfig（读 .env）— 全局配置中心
├── admin_cli.py              # 运维 CLI（status/restart/diag/fix/send...）
├── admin_group_control.py    # 群类型/订阅管理 CLI
├── campus_search.py          # 校园通知搜索（CAS + WebVPN）
├── campus_daily.py           # 校园通知每日推送（scheduler 子进程调用）
├── score_query.py            # 录取分数/位次查询（zs.whut.edu.cn API）
├── zs_plan_query.py          # 招生计划查询
├── zs_whut_search.py         # 招生网全站搜索 + 知识库校验
├── whut_score_api.py         # 分数 API 直连版 + 知识库年份校验
├── smart_search.py           # 7 源聚合智能搜索（可信度加权）
├── searxng_proxy.py          # 本地网页搜索代理（:8899，搜狗/360/必应）
├── scholar_search.py         # 学术论文搜索（OpenAlex，9 源宣称/1 源实现）
├── resource_search.py        # 私有资源站搜索
├── kb_manage.py              # ★ 知识库 CRUD + 多策略搜索
├── kb_semantic.py            # 语义搜索（TF-IDF 分块）
├── kb_search.py              # 兼容薄封装
├── rebuild_kb_index.py       # 一键重建三类索引
├── exam_countdown.py         # 考试倒计时（SQLite）
├── timed_msg.py              # 一次性/循环定时消息
├── alarm_manager.py          # 闹钟 CLI
├── notify_classmate.py       # 通讯录查找
├── docx_export_helper.py     # 文献 .docx 导出
├── xiaonai_doc_tools_v2.py   # 通用文档读写/转换
├── strip_md.py / strip_md_v2.py  # 消息清洗/脱敏/去思维链
├── self_test.py              # L1/L3 自检
├── bridge_health.py          # 健康检查（JSON）
├── proactive_check.py        # 主动巡检（闹钟派发）
├── health_check.sh           # cron 健康自愈（17 项检查）
├── session_cleaner_v2.py     # OpenClaw 会话安全清理
├── .env.example              # 环境变量模板
├── pyproject.toml            # 工具链配置（ruff/pytest/mypy）
├── scripts/                  # systemd 单元 + scan_secrets.sh
├── docs/                     # 架构/部署/保姆教程/功能手册/人格/知识库指南
└── src/
