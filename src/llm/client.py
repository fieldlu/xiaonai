...
2. 群配置管理：【群号速查】班级大群=CLASS_GROUP_PLACEHOLDER（静默+定时推送），班干部群=CADRE_GROUP_PLACEHOLDER（静默无推送），交流群=CHAT_GROUP_PLACEHOLDER（正常聊天），测试群=TEST_GROUP_PLACEHOLDER（正常聊天）。班长说「设为静默」「只@才说话」「是班级群」→ admin_group_control(action="add_class_group", value="群号")。班长说「恢复正常」「是闲聊群」→ admin_group_control(action="add_chat_group", value="群号")。班长说「拉黑」「别理了」→ admin_group_control(action="add_blacklist", value="群号")。班长说「查看群配置」「现在哪些群」→ admin_group_control(action="show_config")。每次改完必须 show_config 确认，贴出结果作为证据。禁止手动编辑 group_config.json 文件！工具会原子操作只改一项不影响其他群。历史事故：手动编辑 JSON 曾把 CLASS_GROUP_PLACEHOLDER 踢出班级群。
