...
# ============================================================
# 学生相关性过滤（09-02c 数据驱动重写 → 09-02f 扩充 → 09-02g 全量金标准回归：
# 1032 条全列人工金标准（gold_full.json）驱动，规则层 FP 38→0、acc 0.947→0.979；
# 此前的"251 条标注回归 acc=100%"已被全量回归证伪——那只是小样本盲区。
# 残余(规则不命中)全部交 NB，NB 训练集已并入全量残余重训至 303 条，
# 端到端(规则+NB) acc=0.999、FN=0、唯一 FP 为两轮人工分歧边界项 247。）
# 综合信息网是全校门户，大量通知面向教职工/行政（工会、人事、采购、基建…），
# 班群受众是学生，这些推送全是噪音。过滤管线分层：
#   1) 硬黑名单（教师/职工/行政噪音词；学生荣誉词经 WHITELIST_OVERRIDE 豁免）
...
#    （09-02g 全量金标准补词：1032 条全列人工复核后，把借强白词/部门前缀
#      误放行的教职工内容逐词收网，以下每个词都在全列核过零学生冲突）
BLACKLIST = (
    ...
    "思政工作",
    # 全量金标准复核收网词（09-02g）：借"本科/开学/大赛/学位/校园网/水电/
    # 国际合作/创新创业/研究生"等宽强白词或部门前缀漏进的学生无关内容。
    # 每词已在 1032 条全列核验：命中样本全部面向教职工/行政，零学生冲突。
    "附属小学", "后期资助", "办公自动化", "留学人员", "国际合作科学计划",
    "教师自定义", "需求征集", "安全大检查",
    # "应用案例征集"（09-02g 收窄）：宽词"案例征集"会连带误伤研究生院的学生向
    # "主题案例征集结果公示"（研究生=强白，先于黑名单前的旧词命中会漏放）。
    # 本词仅命中 本科生院"教师人工智能应用案例征集"（教师向），已全列核验。
    "应用案例征集",
    # POS 部门摘表配套收词（09-02g）：财务处/后管处从 POS_DEPTS 摘除后，其
    # 学生相关通知全由强白词覆盖（缴费/停水/停电/校园卡…），以下行政向措辞
    # 逐词全列核验零学生冲突，直接收网不交 NB：
    "节约用电用水",   # 后管处 高温/寒假"节约用电用水"温馨提示（家属区/办公向）
    "暑期工作安排",   # 财务处 暑期值班安排（教职工向）
    # NB 残余顽固误判收网词（09-02g 第二批）：POS 摘表后这些行政向标题落入 NB 兜底，
    # 字符 NB 分不开"校园网升级维护(keep,影响宿舍)/核心数据库升级(drop,后台)"这类
    # 近重复，LOO-CV 实测仍误推。逐词全列核验零学生冲突（gold=1 零命中）后直接收网：
    "固定资产",       # 财务处 固定资产入库/清查
    "大扫除",         # 后管处 全校卫生大扫除
    "飞絮",           # 后管处 法桐飞絮治理
    "办公用房",       # 后管处 行政办公用房清查
    "报修系统",       # 网络中心 网络工单报修系统停机维护
    "核心数据库",     # 网络中心 后台数据库升级（区别于校园网设备升级,那类关学生上网）
    "本地版",         # 网络中心 Deepseek本地版系统升级（非学生主流服务）
    "应用发布",       # 网络中心 "近期热点"栏目内容发布申请（部门管理员向）
    "流程审核",       # 网络中心 办事大厅流程审核人员调整（内部人事）
    "委托业务费",     # 财务处 行政经费管理
    "会议调整",       # 党政办 内部会议改期（学生大型活动改期标题用"时间调整"不用此词）
    "因公赴",         # 港澳台办 因公赴港澳通行证（教职工因公出访；学生港澳交流自行办证）
    "审计",           # 审计处 政策宣贯/整改培训
    "住宅楼",         # 后管处 家属区住宅楼公共通道/电动车清理
    "防冻",           # 后管处 室外水管防冻保温（设施维护；学生宿舍停水停电由强白词先覆盖）
    "树木修剪",       # 后管处 校园绿化修剪
    "污水管道",       # 余区管委会 污水管道改造（学生宿舍改扩建类标题含"学生宿舍"强白先行）
    "合同管理系统",   # 党政办 OA 合同系统电子签章上线（部门签合同用；学生成绩单电子签章
                      # 153 在研究生院名下且无"合同管理系统"字样，不受影响）
    # 研究生受众屏蔽（09-07，FieldLu：「帮我校园通知屏蔽研究生的」）：本群为本科班群，
    # 面向在读研究生的通知（研究生院竞赛/培养/学籍/学位/导师事务、博士生专项、MPAcc 专硕
    # 大赛等）一律不推。三词全列核验：1032 条中命中 109 条，豁免层（WHITELIST_OVERRIDE）
    # 精确保住 8 条本科生刚需（保研推免/支教团/本科生缴费/本科生选修研究生课程/考研交通
    # 管制）后，其余 101 条全部面向研究生受众，gold 已同步翻转 1→0（67 条）。
    "研究生",         # 含 硕士研究生/博士研究生/研究生院发布的研究生事务（子串覆盖）
    "博士",           # 含 博士生专项计划/博士后（博士后原已单独在表，此处子串合并）
    "MPAcc",          # 会计专硕案例大赛（标题不含"研究生"字样的漏网项）
    # 会议类纵深词（09-07，与 OA 域名拦截配套）：gztz 全列 1032 条核验零命中，
    # 防未来其他栏目混入行政例会/联席会通知（周例会本体已由 ioa 域名拦截根治）。
    "例会", "党政联席",
)
...
WHITELIST_STRONG = (
    ...
    # "封闭/叮叮车"（09-02g 补）：全列核验——"封闭"19 条命中全为基建处/余区管委会
    # 道路或施工区域封闭（gold 全 1，其中 18 条已含"临时封闭"、417 条为"路段封闭"
    # 需此词）；"叮叮车"仅保卫处校园通勤车 1 条（199），NEGDEPT 保卫处硬否决误杀。
    "临时封闭", "封闭", "占道", "打围", "交通管制", "道路维修",
    "叮叮车",
    # 课程 / 学业
    ...
    # 思政/安全教育（09-02g 补）：全民国家安全教育日系列活动、思政公开课等
    # 面向学生的主题教育活动由学工系统组织参加（408/617 全列核验 gold 全 1；
    # 教师向的公开课/示范课标题均带"教学/观摩/教改"等黑词先行拦截）。
    "公开课", "安全教育",
)

# 黑名单豁免（09-02g 补）：个别学生荣誉评选标题含黑名单词（"党员"→组织部
# "评选学生优秀共产党员"），而黑名单先于强白执行会误杀。此表命中则跳过黑名单
# 继续正常判定（后续"学生"强白会放行）。全列核验仅 491 一条命中，零副作用。
WHITELIST_OVERRIDE = (
    "学生优秀共产党员",
    # 研究生受众屏蔽的本科生刚需豁免（09-07，先于黑名单；逐词全列核验仅命中下述
    # 条目且 gold 全 1，零冲突——黑名单"研究生/博士"会误杀它们）：
    "免试攻读",                   # 本科生推免保研（推荐优秀应届本科毕业生免试攻读硕士）
    "支教团",                     # 研究生支教团=应届本科毕业生保留学资项目，本科生可报
    "入学考试",                   # "研究生入学考试交通管制"——考研初试考点通行通告，本科生考生刚需
    "本科生、研究生网上缴费",     # 财务处缴费通知（本科生同缴）
    "本科生选修研究生课程",       # 本科生选修研究生课程（含缴费）通知
)

# 4) 弱白名单：宽泛词，放在部门否决之后
#   （"暑假/课程/放假"在行政、科研部门通知里是噪音，不能先于部门否决放行；
#     档案馆/工会的"放假/值班安排"被 NEG_DEPTS 拦，党政办/无部门的全校放假仍放行。
#     09-02g 摘"网络"：网络中心每条标题都自带部门前缀"网络"二字，此前所有
#     网络中心通知无论内容都被此词截胡 keep，POS_DEPTS 从未生效。摘除后其
#     学生向(正版软件/网络中断/风险提醒等)与行政向(IT运维/内部流程)一并交 NB 判定)
WHITELIST_WEAK = ("暑假", "寒假", "暑期", "课程", "放假")
...
#    09-02g 摘除 财务处/后管处/医管办/网络中心：全量金标准回归显示其 POS 默认放行
#    制造全部 19 个 POSDEPT 层误推（报销/清查/倡议/职工医疗/IT 运维）。核对后发现
#    这四部门的学生相关通知 100% 由强白词先行覆盖（缴费/退费/医保/停水/停电/水池/
#    校园网/校园卡/邮箱…），无强白词的均为教职工/行政向——摘表后行政项落入黑词或
#    NB，学生项不受影响（网络中心 10 条服务类如"正版软件上线/钓鱼提醒"无强白词，
#    交 NB 后由 09-02g 重训数据兜底）。
POS_DEPTS = (
    "图书馆", "后勤集团", "医院", "体育学院", "团委", "学工部",
)
...
def classify(title):
    ...
    # 黑名单豁免先于黑名单(09-02g)：见 WHITELIST_OVERRIDE 注释
    skip_black = any(w in title for w in WHITELIST_OVERRIDE)
    if not skip_black:
        for w in BLACKLIST:
            if w in title:
                return 'drop'
...
# 训练集 campus_notice_labels.json：1034 条（09-02h 全量训练版）——1032 条收割全列
# 金标准 gold_full.json 去重后 960 条唯一标题全部并入训练（残余 80 条排前 + 规则层
# 880 条，杜绝"只学残余、漏学全量"）+ 74 条历史补标。排除式验证（每残余样本从
# 训练集剔除后预测）：残余部署池误差 5/80→4/80，全量样本为行政/学生文体提供同文体
# 证据；仅 247（两轮人工分歧）/345+984（校领导接待日）/872（扫雪倡议）为 NB 上限外。
# 每次运行内存内训练一次（毫秒级），零外部依赖。
# 超参数 (1,2,3)-gram / alpha=0.5 / theta=1.5（09-02f 定标 → 09-02h 全量训练重定标：
# 1) (2,3)→(1,2,3) 是 CV 稳定增益（macroF1 0.8390→0.8455±0.013，keepR 0.876→0.889）；
# 2) α∈{0.1,0.3,0.5} 定标，取最大 α=0.5 以平滑罕见字共现、降低近重复行政措辞过拟合；
# 3) θ 2.5→1.5：全量训练先验近 50/50 抬高 log-odds 基线，排除式扫 θ∈[1.0,4.5] 最优为
#    1.0~1.5（误差 4/80），取 1.5 更保守；in-sample keep 全 ≥30、drop 除 247 外 ≤-19.7。
# 残余 drop 58 条在 θ=1.5 下仍 100% 判对，θ 下调不引入 drop→keep 误放）。
NOTICE_LABELS_FILE = Path(__file__).resolve().with_name('campus_notice_labels.json')

import math as _math

# NB 超参数（09-02h 全量训练排除式定标，勿随意改）
...
def fetch_notice_list():
    ...
    for a in soup.select('a[href]'):
        href = a.get('href', '')
        # OA 内部办公域拦截（09-07，FieldLu：「还有第X周会议，这个没什么用」）：
        # ioa.whut.edu.cn 的 Seeyon OA 动态链接（每周工作例会 NewWeekMeeting 等）
        # 是教职工/行政内部事务，每周一个新链接反复混进推送；标题动态生成无固定词
        # 可拦（收割记录 title 为空），按域名在抓取阶段直接丢弃。gztz 全列 1032 条
        # 核验：OA 域名零命中，无学生内容误杀风险。
        if 'ioa.whut.edu.cn' in href:
            continue
